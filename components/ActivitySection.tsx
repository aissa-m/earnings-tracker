...
import { BriefcaseBusiness, Check, FileText, Layers3, PencilLine, RotateCcw, Sparkles, Trash2 } from "lucide-react";
...
function getEditableProjects(projects: Project[], entry: Entry) {
  return projects.filter((project) => entry.work_type === "Labeling" ? project.supports_labeling : project.supports_reviewing);
}
...
export default function ActivitySection({ entries, projects, onDelete, onSaved }: { entries: Entry[]; projects: Project[]; onDelete: (id: string) => void; onSaved: () => Promise<void> }) {
  ...
  function startEdit(entry: Entry) {
    ...
    setEditForm(getInitialEditForm(projects, entry));
  }
  ...
  async function saveEdit(entry: Entry) {
    ...
    const amount = entry.unit === "DR" ? Number(editForm.amount) : Number(editForm.hours || 0) + Number(editForm.minutes || 0) / 60;
            ...
            return (
              <article key={entry.id} className="mini-card">
                ...
                <div className="mt-3 flex flex-wrap justify-end gap-2">
                  {isEditing ? (
                    <>
                      <button onClick={() => saveEdit(entry)} disabled={savingId === entry.id} className="inline-flex items-center gap-1 rounded-full bg-brand px-3 py-1.5 text-xs font-black text-slate-950 transition active:scale-[0.98]"><Check size={14} /> {savingId === entry.id ? "Guardando..." : "Guardar"}</button>
                      <button onClick={cancelEdit} className="inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-black text-slate-600 hover:bg-slate-950/5"><RotateCcw size={14} /> Cancelar</button>
                    </>
                  ) : (
                    <button onClick={() => startEdit(entry)} className="inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-black text-slate-600 hover:bg-slate-950/5"><PencilLine size={14} /> Editar</button>
                  )}
